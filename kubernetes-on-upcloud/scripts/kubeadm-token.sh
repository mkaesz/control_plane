...
eval "$(jq -r '@sh "HOST=\(.host) USER=\(.user) PASSWORD=\(.password)"')"

# Fetch the join command
CMD=$(sshpass -p $PASSWORD ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null $USER@$HOST kubeadm token create --print-join-command)
