...
function add_os_deps() {
	sudo -E swupd bundle-add --quiet cloud-native-basic storage-utils jq bc
}
...
function enable_kubelet_runner() {
	# Get Upcloud server's private IP address
	LOCAL_IP=$(ip a | grep "inet 10" | cut -d" " -f6 | cut -d"/" -f1)
	mkdir -p /etc/default/

	cat <<EOT | sudo bash -c "cat > /etc/default/kubelet"
KUBELET_EXTRA_ARGS=--node-ip=$LOCAL_IP --address=$LOCAL_IP
EOT
